    ...
    handleDraw =()=>{
		...
		function initDraw (canvas,addcallback,removecallback) {
			...
			canvas.onclick = function (e) {
				...
				if(numRect > 0 && (document.getElementById('label'+numRect) && document.getElementById("label"+numRect).value === "" || !okTrigger) ||  buttonTrigger === 1){
					if(buttonTrigger ===1){
						console.log("empty")
	...
	handleRemoveLabel = (num)=>{
		this.setState(state=>{
			...
			state.label.map((l,index)=>{
				if(num === l.labelno){
					labArray = labArray.slice(0, index).concat(labArray.slice(index+1, labArray.length))
				}
			})
			return{
				label:labArray
...
export default connect(mapStateToProps, mapDispatchToProps)(ToolIndex)